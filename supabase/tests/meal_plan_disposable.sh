...
umask 077
TMP=$(mktemp -d /tmp/ck-meal.XXXXXX)
cleanup() {
  local status=$?
  trap - EXIT
...
if [[ ${1:-} != --red ]]; then
 "${PSQL[@]}" -f "$ROOT/supabase/migrations/20260914000200_atomic_meal_plan_replacement.sql"
 "${PSQL[@]}" -f "$ROOT/supabase/migrations/20260920000100_coherent_meal_plan_read.sql"
fi
"${PSQL[@]}" -f "$ROOT/supabase/tests/meal_plan_replacement.sql"
if [[ ${1:-} != --red ]]; then
 "${PSQL[@]}" -f "$ROOT/supabase/tests/meal_plan_read.sql"
 bash "$ROOT/supabase/tests/meal_plan_concurrency.sh"
 bash "$ROOT/supabase/tests/meal_plan_read_concurrency.sh"
fi
