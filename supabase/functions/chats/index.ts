...
async function getUserContext(authHeader: string | null): Promise<{ supabase: SupabaseClient; userId: string } | Response> {
    ...
    if (error) {
        console.error("[chats] Auth verification failed:", error.message);
        return respondJson({ code: 401, message: "Invalid JWT" }, 401);
    }

    if (!data.user) {
        console.error("[chats] No user returned from auth verification");
        return respondJson({ code: 401, message: "Invalid JWT" }, 401);
    }

    console.log("[chats] User verified:", data.user.id);
