...
- **Pre-release read-only production check:** the g11-inventory §0 FK-to-`auth.users` query, to rule out a `NO ACTION`/`RESTRICT` FK outside the purged tables that would make deletion return `AUTH_DELETE_FAILED`.
...
- **Deferred nits (unchanged from before G–J):**
  ...
  - Confirm-branch analysis errors (404/409/410) still collapse to one generic message (I4 flagged this as out of scope for the pass).
